...
def sent_token(sentence_tokenized):
    ...
    for i in range(len(sentence_tokenized)):
        ...
        if (len(attr4) == len(attr5) == len(attr6) == len(attr7) == len(attr8) == len(attr9) == len(attr47) == len(attr48) == len(attr52_53) == 0):
            print(1)
            new_attr = new_tr.newTr(feed,POS,PARSED,TOKENIZED)
            if (len(new_attr)>1) :
                finalgraph.draw([],new_attr,attr4,attr5,attr6,attr7,attr8,attr9,attr47,attr48,attr52_53,para_length)
            else:
...
def parsing_tokinizing(sentence):
    nlp = StanfordCoreNLP(r'C:\Program Files (x86)\Python37-32\stanford-corenlp-full-2018-10-05')
    tokenized = ['ROOT-0']
